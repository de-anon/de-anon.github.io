        ...
        socials.forEach(social => {
            ...
            if (social.url) {
                ...
                item.addEventListener('click', (e) => {
                    e.preventDefault();
                    if (social.name === 'Instagram' || social.name === 'Копировать ссылку') {
                        navigator.clipboard.writeText(window.location.href).then(() => {
                            alert(social.name === 'Instagram' 
                                ? 'Ссылка скопирована! Откройте Instagram и вставьте её в сообщение.'
            ...
            wrapper.style.cssText = 'width:44px;height:44px;border-radius:50%;overflow:hidden;display:flex;align-items:center;justify-content:center;background:#00ff41;transition:background 0.3s ease;margin:0 auto;';
            wrapper.innerHTML = social.svg;
            const svgEl = wrapper.querySelector('svg');
            if (svgEl) {
                svgEl.style.width = '44px';
                svgEl.style.height = '44px';
                svgEl.style.display = 'block';
            }
            item.appendChild(wrapper);

            // Запоминаем wrapper для hover
            ...
            // При наведении меняем фон круга на оригинальный цвет
            item.addEventListener('mouseenter', function() {
                this._wrapper.style.background = this.getAttribute('data-color');
            });
            item.addEventListener('mouseleave', function() {
                this._wrapper.style.background = '#00ff41';
            });
